...
def proposed_experiments(features, activation='swish'):
    ...
    b1 = conv3x3(x, 256, 'aspp1',
                    rate=atrous_rates[0], epsilon=EPSILON, activation=activation, depth_activation=True)
    # rate = 12 (24)
    b2 = conv3x3(x, 256, 'aspp2',
                    rate=atrous_rates[1], epsilon=EPSILON, activation=activation, depth_activation=True)
    # rate = 18 (36)
    b3 = conv3x3(x, 256, 'aspp3',
                    rate=atrous_rates[2], epsilon=EPSILON, activation=activation, depth_activation=True)
    # concatenate ASPP branches & project
    x = Concatenate()([b4, b0, b1, b2, b3])
    ...
    # edge create branch
    edge_branch = conv3x3(c3, 256, 'edge_conv1', stride=1, kernel_size=3, rate=1, depth_activation=True, activation='swish')
    edge_branch = conv3x3(edge_branch, 256, 'edge_conv2', stride=1, kernel_size=3, rate=1, depth_activation=True, activation='swish')

    edge_branch = Subtract()([x, edge_branch])
    spatial_map = spatial_attention(edge_branch)
    edge_1 = multiply([spatial_map, edge_branch])

    x = Add()([x, edge_1])

    x = conv3x3(x, 256, 'conv_128x_1', stride=1, kernel_size=3, rate=1, depth_activation=True, activation='swish')
    x = UpSampling2D(size=(2, 2), interpolation='bilinear')(x)

    spatial_map = UpSampling2D(size=(2, 2), interpolation='bilinear')(spatial_map)
    # here is edge (spatial_map)
    edge_map = spatial_map

    edge_2 = multiply([spatial_map, x])
    ...
    x = conv3x3(x, 256, 'conv_128x_conv', stride=1, kernel_size=3, rate=1, depth_activation=True, activation='swish')
    # here is body (x)
    body_map = x

    x = Add()([x, edge_2])

    x = conv3x3(x, 256, 'final_conv1', kernel_size=3, epsilon=EPSILON, activation=activation, depth_activation=True)
    x = conv3x3(x, 256, 'final_conv2', kernel_size=3, epsilon=EPSILON, activation=activation, depth_activation=True)

    return x, edge_map, body_map
